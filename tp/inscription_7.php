...
function afficher_formulaire($valPseudo='', $valNom='', $valMail='', $valJ=0,$valM=0,$valA=0){
	...
	$submit=KG_html_form_input('submit','btnValider','Je m\'inscris',5);
	$submitButton=KG_html_table_ligne('',$submit);
	echo '<h1>Inscription d\'un utilisateur</h1>',
	'<form method="POST" action="inscription_7.php">',
		'<table>';
			echo $Pseudo;
			echo $Passe;
			echo $Passe2;
...
if(isset($_POST['btnValider'])==false){
	...
	if(count($array)!=0){
		...
		header('location: liste_users_2.php');
	}
	$jour=(int)$_POST['selNais_j'];
    $mois=(int)$_POST['selNais_m'];
	$annee=(int)$_POST['selNais_a'];
	afficher_formulaire(protect($_POST['txtPseudo']),protect($_POST['txtNom']),protect($_POST['txtMail']), $jour,$mois,$annee);
}
//fin du html
KG_aff_fin();
exit;
