...
if($G['COUNT(usID)']==0){
	header('location: utilisateur.php?id='.$usID);
}
...
//header du html
if($_SESSION['id']==$usID){
	KG_aff_entete('n','Mes mentions');
}else{
	KG_aff_entete('n','Les mentions de '.protect($B['usPseudo']));
}
...
if($nombrebl==0){
	...
	KG_aff_blablas($bd,$existbl,$nbblablas, 'mentions.php',$nombrebl ,$usID);
}else{
	...
	//affichage des blablas
	KG_aff_blablas($bd,$sqlblablas,$nbblablas , 'mentions.php', 1 , $usID);
}
